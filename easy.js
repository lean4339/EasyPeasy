...
http.createServer((req, res) => {
	...
	switch (req.url) {
		...
		case '/':
			...
			res.write("\n\n")
			let titulos = [];
			res.write(`el total de peliculas es ${movies.length}\n`)
			res.write("\n\n")
			for(let i = 0; i< movies.length; i++){
				titulos.push(movies[i].title)
			}
			titulos.sort()
			...
			res.write("\n\n")
			res.write("Recordá que podes visitar las secciones: \n")
		...
		case '/en-cartelera':
			...
			res.write("\n\n")
			for(let i = 0; i < movies.length; i++){
				res.write(`Titulo: ${movies[i].title.toUpperCase()}\n`)
				res.write(`Reseña: ${movies[i].overview}`)
				res.write("\n\n")
			}
			res.end();
			break;
		case '/mas-votadas':
			...
			res.write("\n\n")
			masVotadas = movies.filter(function (objeto){
				return objeto.vote_average >= 7
			})
			res.write(`Total de peliculas: ${masVotadas.length}`)
			res.write("\n\n")
			let sumatoria = 0;
			masVotadas.forEach(element => {
					sumatoria += element.vote_average				
			});
			res.write(`Rating promedio ${(sumatoria/masVotadas.length).toFixed(2)}`)
			res.write("\n\n")
			masVotadas.forEach(element => {
		...
		case '/sucursales':
			...
			res.write("\n\n")
			res.write(`Total de salas: ${theaters.length} salas`)
			res.write("\n\n")
			for(let i = 0; i < theaters.length; i++){
				res.write(`Nombre de sala: ${theaters[i].name}\n`)
				res.write(`Direccion de sala: ${theaters[i].address}\n`)
				res.write(`Descripcion de la sala: ${theaters[i].description}\n`)
				res.write("\n\n")
			}
			res.end();
			break;
		case '/contacto':
			res.write("Contacto")
			res.write("\n\n")
			res.write("Tenes algo para contarnos? Nos encanta escuchar a nuestros clientes. Si deseas contactarnos podes escribir al siguiente email: dhmovies@digitalhouse.com o en las redes sociales. Envianos tu consulta, sugerencia o reclamo y será respondido a la brevedad posible. Recordá que tambien podes consultar la seccion Preguntas Frecuentes para obtener respuestas inmediatas a los problemas mas comunes.")
			res.end();
			break;
		case '/preguntas-frecuentes':
			...
			for(let i = 0; i < faqs.length; i++){
				res.write(`Titulo : ${faqs[i].faq_title}\n`)
				res.write(`Respuesta: ${faqs[i].faq_answer}\n`)
				res.write("\n\n")
			}
			res.end();
			break;
...
}).listen(3000, 'localhost', () => console.log('Server running in 3000 port'));